    ...
    return new Promise(function(resolve, reject) {
      ...
      if (_.has(options, "admin") && !_.has(options, "input")) {
        ...
        var logging = new Logging({
          projectId: options.instance,
          keyFilename: options.admin,
        });

        var logName = "realtime-database";
        var log = logging.log(logName);
        var metadata = { resource: { type: "global" } };
        var logWrites = [];
        generateReport = () => Promise.all(logWrites).then(() => resolve(null));
      } else {
        ...
        generateReport = _.once(function() {
          rl.close();
          spinner.stop();
          if (erroring) {
      ...
      request
        ...
        .on("data", function(chunk) {
          ...
          if (loggingEnabled) {
            var dataString = chunk.toString().split("\n")[1];
            var dataObject = ProfileReport.extractJSON(dataString, false);

            if (dataObject) {
              if (dataObject.path) {
                dataObject.path = dataObject.path.join("/");
              }
              logWrites.push(log.info(log.entry(metadata, dataObject)));
            }
          } else {
